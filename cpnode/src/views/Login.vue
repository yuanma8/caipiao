	...
	<div class="login">
		...
		<div class="login-wrap">
			<ul>
				...
				<li class="reg"><a class="blue" @click="visitorLogin()">免费试玩</a>
					<router-link to="register" class="red">立即注册</router-link>
				</li>
				<li class="forget">
		...
		methods: {
			...
			login() {
				if (this.body.username === '' || this.body.password === '') {
					this.$alert.show('用户名或密码不能为空!');
					return;
				}
				this.$loading.show('登录中');			
				this.$http.post('user/login', this.body, {
					...
					.then(res => {
						...
						this.$setCookie('chatLevelTypeName', res['chatLevelTypeName'], 1000 * 60 * 60 * 24 * 30);
						this.$setCookie('levelName', res['levelName'], 1000 * 60 * 60 * 24 * 30);
						this.$setCookie('userLevel', res['userLevel'], 1000 * 60 * 60 * 24 * 30);
						// 一个月
...
<style scoped>
	...
	.yanzhen i {
		...
		background: url(../assets/logn_account.png);
		background-size: 100%;
		margin-right: 1rem;
	...
	.pwd i {
		background: url(../assets/logn_passport.png);
		background-size: 100%;
	}
	...
	.eye-on {
		width: 2.2rem;
		height: 2.2rem;
		background: url(../assets/eye_off.png) center;
		background-size: 100%;
		display: inline-block;
	}

	.eye-on {
		background: url(../assets/eye_on.png) center;
		background-size: 100%;
	}
