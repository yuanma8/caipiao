...
use think\Request;
class BalanceController extends ComController
{
    public function index()
    {
       session_start();
        $uid = session('uid');
        $msg=$uid;
        echo json_encode(array('msg'=>$msg,'IsSuccess'=>true,'StatusCode'=>'0'));
        //ThinkPHP代码片段
        if(IS_POST)
        {
            $lotteryId =$_POST['lotteryId'];
            $model = M("user");
            $user = $model->field('uid,username')->where(array('username' => $username, 'password' => $password))->find();
           /*
            if ($user) {
                $salt = C("COOKIE_SALT");
                $ip = get_client_ip();
                ...
                //加密cookie信息
                $auth = password($user['uid'].$user['username'].$ip.$ua.$salt);
                if ($remember) {
                    cookie('auth', $auth, 3600 * 24 * 365);//记住我
                } else {
                    cookie('auth', $auth);
                }
                echo json_encode(array('msg'=>'success.','IsSuccess'=>true,'StatusCode'=>'0'));
                addlog('登录成功。');
                $url = U('index/index');
                //  header("Location: $url");
                exit(0);
               ...
               echo json_encode(array('msg'=>'Some errors occured.'.$password,'IsSuccess'=>true,'StatusCode'=>'0'));
            }
            */
            
            $file  = 'c:\users\beimei\cp\log.txt';//要写入文件的文件名（可以是任意文件名），如果文件不存在，将会创建一个  
            if($f  = file_put_contents($file, $data,FILE_APPEND)){// 这个函数支持版本(PHP 5) 
            }
